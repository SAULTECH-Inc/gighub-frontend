...
const JobPostingStatus = () => {
  const { employerSettings, setJobPostingStatus, updateJobPostingStatus } =
    useSettingsStore();

  // Get the actual data from employerSettings instead of the separate state
  const actualJobPostingStatusData =
  ...
  useEffect(() => {
    ...
    if (
      jobPostingStatusData &&
      (!actualJobPostingStatusData ||
        JSON.stringify(actualJobPostingStatusData) !==
          JSON.stringify(jobPostingStatusData))
    ) {
  ...
  const handleJobPostingToggle = useCallback(
    (key: string) => {
      ...
      const updatedSettings = {
        ...
        option: {
          ...
          [key]:
            !actualJobPostingStatusData.option[
              key as keyof JobPostingStatusNotificationOptions
            ],
        },
      };
    ...
    <section className="font-lato flex w-[95%] flex-col self-center py-10 md:w-[90%]">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600">
          Track the lifecycle of your job postings from draft to publication and
          beyond.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              {jobPostingOptions.map((option) => {
                ...
                const isActive =
                  actualJobPostingStatusData?.option?.[
                    option.key as keyof JobPostingStatusNotificationOptions
                  ] || false;
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex flex-1 items-start space-x-3">
                        <div
                          className={`rounded-lg p-2 transition-colors duration-200 ${isActive ? "bg-amber-100 text-amber-600" : "bg-gray-100 text-gray-500"} `}
                        >
