...
import {
  applicantNavBarItemMap, applicantNavItems, applicantNavItemsMobile
} from "../../utils/constants";
import { APIResponse, EmployerData } from "../../utils/types";
import { fetchCompanies, followCompany, unfollowCompany } from "../../services/api";
...
const usePaginationParams = () => {
  const [params, setParams] = useState<PaginationParams>({
    page: 1, limit: 20, sortBy: {
      companyName: "", industry: "", location: "", employeeCount: "", foundedYear: "", rating: "", openPositions: ""
...
const CompanyList = () => {
  const { params: pagination, setParams: setPagination, stableParams } = usePaginationParams();
  ...
  const [viewedCompanies, setViewedCompanies] = useState<Set<number>>(new Set());
  const [searchQuery, setSearchQuery] = useState("");
  const [isExporting, setIsExporting] = useState(false);
  const [aggregations,] = useState<ApiResponse["aggregations"]>();
  const [processingFollow, setProcessingFollow] = useState<Set<number>>(new Set());
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setPagination(prev => ({
        ...prev, searchQuery: searchQuery, page: 1
      }));
    }, 300);
  ...
  const fetchCompaniesData = useCallback(async () => {
    ...
    try {
      const response: APIResponse<EmployerData[]> = await fetchCompanies(pagination);
  ...
  }, [stableParams.page, stableParams.limit, stableParams.sortDirection, stableParams.searchQuery, stableParams.sortBy, stableParams.filters]);
  ...
  const toggleFollowCompany = useCallback(async (companyId: number) => {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        toast.success(wasFollowing ? "Company unfollowed successfully" : "Company followed successfully");
      } else {
        ...
        toast.error(response.message || `Failed to ${wasFollowing ? 'unfollow' : 'follow'} company`);
      }
    } catch (error: any) {
  ...
  const PaginationButton = ({
                              onClick, disabled, children, isActive = false, className = "", title
                            }: {
    onClick: () => void;
    disabled?: boolean;
    ...
    className={`
        ...
        ${isActive ? "bg-purple-600 text-white border-purple-600 shadow-sm" : "bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-400"}
  ...
  const LoadingSkeleton = () => (<div
    ...
    {[...Array(pagination.limit)].map((_, index) => (<div key={index} className="animate-pulse">
  ...
  return (<div className="min-h-screen bg-gray-50">
    ...
    <div className="bg-[#F7F8FA] p-4">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-300 bg-white py-3 pl-12 pr-4 text-gray-900 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20"
              />
              {searchQuery && (<button
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className={`inline-flex items-center space-x-2 px-4 py-3 rounded-lg border transition-all ${showFilters || activeFiltersCount > 0 ? "bg-purple-600 text-white border-purple-600" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50"}`}
              disabled={isLoading}
            >
              <FiFilter className="h-4 w-4" />
              <span>Filters</span>
              {activeFiltersCount > 0 && (<span className="bg-white/20 text-xs px-2 py-1 rounded-full">
                    {activeFiltersCount}
                  </span>)}
            </button>
            ...
            <button
              ...
              className="inline-flex items-center space-x-2 px-4 py-3 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
      ...
      <div className="mt-5 flex items-start gap-x-6 max-lg:flex-col-reverse">
        <div
          className="flex w-full flex-col justify-center rounded-2xl bg-white p-6 shadow-md border border-gray-100">
          ...
          <div className="mb-6">
            ...
            {/* Enhanced Toolbar */}
            <div
              className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <span className="text-sm text-gray-600">
                    {isLoading ? (<span className="flex items-center">
                      ...
                      Showing <span className="font-semibold">{getStartIndex}-{getEndIndex}</span> of{" "}
                      <span className="font-semibold">{totalCompanies.toLocaleString()}</span> companies
                    </>)}
                  </span>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="inline-flex items-center px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 transition-all"
                  title="Refresh data"
                >
            ...
            {activeFiltersCount > 0 && (<div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          {/* Empty State */}
          {!isLoading && !error && companies.length === 0 && (<div className="text-center py-12">
          ...
          {!isLoading && !error && companies.length > 0 && (<>
            ...
            {/* Enhanced Pagination */}
            {totalPages > 1 && (<div className="border-t border-gray-200 pt-6">
              <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  {getVisiblePageNumbers.map((pageNum) => (<PaginationButton
                    key={pageNum}
                  ...
                  {pagination.page < totalPages - 2 && totalPages > 5 && (<PaginationButton
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <input
                    ...
                    max={totalPages}
                    placeholder="Page"
                    className="w-20 text-sm border border-gray-300 rounded-md px-3 py-2 text-center focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    onKeyPress={(e) => {
                      if (e.key === "Enter") {
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    {followedCompaniesCount > 0 && (<span className="flex items-center space-x-1">
                            <FiHeart className="h-4 w-4 text-red-400" />
                            <span>{followedCompaniesCount} companies following</span>
                          </span>)}
                  </div>
      ...
      <div className="fixed bottom-6 right-6 lg:hidden flex flex-col space-y-3">
        {followedCompaniesCount > 0 && (<button
          onClick={() => {/* Navigate to saved companies */}}
          className="flex items-center justify-center w-14 h-14 bg-red-600 text-white rounded-full shadow-lg hover:bg-red-700 transition-all"
        ...
        <button
          onClick={() => setShowFilters(!showFilters)}
          className={`flex items-center justify-center w-14 h-14 rounded-full shadow-lg transition-all ${showFilters || activeFiltersCount > 0 ? "bg-purple-600 text-white" : "bg-white text-gray-700 border border-gray-300"}`}
