      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          className={`transition-all duration-200 ${
            readonly
              ? "cursor-default"
              : "cursor-pointer rounded hover:scale-110 focus:ring-2 focus:ring-yellow-400 focus:ring-offset-1 focus:outline-none"
          }`}
  ...
  return (
    <div className="relative">
      <button
        ...
        className="rounded-full p-2 transition-colors hover:bg-gray-100 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 focus:outline-none"
      ...
      {showMenu && (
        <div
          ref={menuRef}
          className="animate-in fade-in slide-in-from-top-2 absolute top-full right-0 z-30 mt-2 w-56 rounded-lg border border-gray-200 bg-white py-2 shadow-lg duration-200"
          role="menu"
        ...
        >
          ...
          <button
            ...
            className="flex w-full items-center px-4 py-3 text-sm text-gray-700 transition-colors hover:bg-gray-50 focus:bg-gray-50 focus:outline-none"
            role="menuitem"
          >
...
const JobDetailItem: React.FC<{
  ...
  <div className="flex items-center gap-2">
    <div className="flex-shrink-0 rounded-lg bg-gray-50 p-1.5">{icon}</div>
    <div className="min-w-0 flex-1">
      <p className="text-[10px] font-medium text-gray-500">{label}</p>
      <p className={`truncate text-[11px] font-semibold ${colorClass}`}>
        {typeof value === "number" && label === "Days left"
          ? `${value}d`
          : value}
      </p>
    </div>
...
export const BestMatchedJobDetails: React.FC<JobMatchCardProps> = ({
  ...
  const companyLogoUrl = useMemo(
    () =>
      companyLogo ||
      `https://ui-avatars.com/api/?name=${encodeURIComponent(company)}&background=6366f1&color=fff&size=40`,
    [companyLogo, company],
  );
  ...
  const daysLeftStyling = useMemo(
    () => getDaysLeftStyling(daysLeft),
    [daysLeft, getDaysLeftStyling],
  );
  ...
  return (
    <article className="relative mx-auto flex h-full w-full max-w-full min-w-[320px] flex-col gap-y-5 rounded-xl border border-gray-100 bg-gray-100 p-2">
      {/* Header Section */}
        ...
        <div className="flex w-full items-start space-x-3">
          ...
          <div className="flex-shrink-0">
            <div className="relative">
              <img
                src={companyLogoUrl}
                alt={`${company} logo`}
                className="h-12 w-12 rounded-xl bg-white object-cover ring-2 ring-gray-100"
                loading="lazy"
                onError={(e) => {
              ...
              />
              {rating >= 4.5 && (
                <div className="absolute -top-1 -right-1 rounded-full bg-gradient-to-r from-yellow-400 to-orange-400 p-1 shadow-sm">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-y-3">
            ...
            <div className="flex flex-col gap-y-1">
              <h2 className="line-clamp-2 cursor-pointer text-base leading-tight font-bold text-gray-900 transition-colors hover:text-indigo-600">
                {title}
              </h2>
            ...
            <div className="flex flex-wrap gap-1.5">
              {visibleTags.map((tag, index) => (
                <span
                  key={`${tag}-${index}`}
                  className="inline-flex cursor-pointer items-center rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200 transition-colors hover:bg-indigo-100"
                >
                  {tag}
                </span>
              ))}
              ...
              {hiddenTagsCount > 0 && (
                <span
                  className="inline-flex cursor-pointer items-center rounded-full bg-gray-50 px-3 py-1 text-xs font-medium text-gray-600 ring-1 ring-gray-200 transition-colors hover:bg-gray-100"
      ...
      {/* Action Buttons */}
      <footer className="flex gap-3 border-t border-gray-100 pt-4">
        <button
          onClick={handleJobView}
          className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition-all duration-200 hover:border-gray-400 hover:bg-gray-50 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:outline-none"
          type="button"
        >
        ...
        <button
          ...
          className="group flex flex-1 items-center justify-center gap-2 rounded-xl border-2 border-transparent bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-3 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:from-indigo-700 hover:to-purple-700 hover:shadow-lg focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0 disabled:hover:from-indigo-600 disabled:hover:to-purple-600 disabled:hover:shadow-none"
          type="button"
        >
          {isApplying ? (
            <>
