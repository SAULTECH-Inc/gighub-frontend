...
export const useDashboardData = (
  ...
  const isSuccessfulResponse = <T>(response: APIResponse<T>): boolean => {
    ...
    return response.statusCode === 200 || (!response.error && !response.statusCode) || (response.statusCode >= 200 && response.statusCode < 300);
  };
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      ...
      if (existingMetrics) {
        try {
          const transformedMetrics = await transformExistingMetrics(existingMetrics);
          setData(prev => ({ ...prev, metrics: transformedMetrics }));
        } catch (transformError) {
          console.error('Failed to transform existing metrics:', transformError);
        }
      }
    } finally {
  ...
  const loadMetrics = useCallback(async (period: string = 'month') => {
  ...
  const loadCandidates = useCallback(async (limit: number = 10) => {
    setLoading(prev => ({ ...prev, candidatesLoading: true }));
    setErrors(prev => ({ ...prev, candidatesError: null }));

    try {
      const response = await fetchTopCandidates(limit, filters.applicationStatus);
      if (isSuccessfulResponse(response)) {
  ...
  const loadInterviews = useCallback(async (days: number = 7) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to load interviews';
  ...
  const loadFunnel = useCallback(async (period: string = 'month') => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to load funnel data';
  ...
  }, [loadingStrategy, filters.period, loadMetrics, loadInterviews, loadCandidates, loadJobs, loadActivity, loadFunnel, loadGoals, loadInsights]);
  ...
  const refreshMetrics = useCallback(() => loadMetrics(filters.period), [loadMetrics, filters.period]);
  const refreshCandidates = useCallback(() => loadCandidates(), [loadCandidates]);
  const refreshInterviews = useCallback(() => loadInterviews(), [loadInterviews]);
  const refreshJobs = useCallback(() => loadJobs(), [loadJobs]);
  const refreshActivity = useCallback(() => loadActivity(), [loadActivity]);
  const refreshFunnel = useCallback(() => loadFunnel(filters.period), [loadFunnel, filters.period]);
  ...
  const transformedMetrics = data.metrics ? transformMetricsData(data.metrics) : [];

  // Computed values
  const totalApplicationsCount = data.topCandidates.length;
  const todaysInterviewsCount = data.upcomingInterviews.filter(interview =>
  ...
  return {
    ...
    updateCandidate: useCallback((candidateId: number, updates: Partial<DashboardCandidate>) => {
      setData(prev => ({
        ...prev,
    ...
    updateInterview: useCallback((interviewId: number, updates: Partial<DashboardInterview>) => {
      setData(prev => ({
        ...prev,
    ...
    updateJob: useCallback((jobId: number, updates: Partial<DashboardJobPosting>) => {
      setData(prev => ({
        ...prev,
