...
const addSkill = async (
  ...
  applicantId: number,
  cvId: number,
): Promise<APIResponse<SkillsResponseDto>> => {
...
export const useAddSkill = () => {
  ...
  return useMutation({
    mutationFn: ({ skill, applicantId, cvId }: AddSkillRequest) =>
      addSkill(skill, applicantId, cvId),
    onSuccess: () => {
...
export const useDeleteSkill = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ applicantId, cvId, skill }: DeleteSkillRequest) =>
      deleteSkill(applicantId, cvId, skill),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["skills"] });
    },
  });
