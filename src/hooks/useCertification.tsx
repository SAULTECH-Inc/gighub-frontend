...
const addCertification = async (
  ...
  applicantId: number,
  cvId: number,
): Promise<APIResponse<CertificationResponseDto>> => {
...
const deleteCertification = async (
  ...
  const response = await privateApiClient.post(
    `/cv/delete-certification/${certificationId}`,
    {
      applicantId,
      cvId,
      certificationId,
...
export const useAddCertification = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      addCertification(certification, applicantId, cvId),
    onSuccess: () => {
...
export const useDeleteCertification = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      deleteCertification(applicantId, cvId, certificationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["Certifications"] });
    },
  });
