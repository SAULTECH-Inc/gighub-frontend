...
interface ChatStore {
    ...
    fetchOneToOneMessages: (sender: string, recipient: string, page?: number, limit?: number) => Promise<ChatMessage[]>;
    fetchGroupMessages: (groupID: number, page?: number, limit?: number) => Promise<ChatMessage[]>;
    fetchPrivateMessages: (sender: string, recipient: string, page?: number, limit?: number) => Promise<ChatMessage[]>;
    fetchAllMyMessages: (sender: string, recipient: string, page?: number, limit?: number) => Promise<ChatMessage[]>;
    ...
    devtools(
        persist(
            immer<ChatStore>((set, get) => ({
                ...
                fetchOneToOneMessages: async (sender, recipient, page = 1, limit = 20) => {
                    try {
                        const response = await privateApiClient.get<APIResponse<ChatMessage[]>>(
                ...
                fetchPrivateMessages: async (sender, recipient, page = 1, limit = 20) => {
                    try {
                        const response = await privateApiClient.get<APIResponse<ChatMessage[]>>(
