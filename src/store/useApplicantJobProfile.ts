...
import {
    ...
    JobPreference,
    JobPreferenceResponse, SkillsResponseDto
} from '../utils/types';
...
interface ApplicantJobProfile {
    ...
    updatedCvDetails: (cvId: number, applicantId: number, data: Partial<CvResponseDto>)=>Promise<CvResponseDto | null>;
    ...
    devtools(
        persist(
            immer<ApplicantJobProfile>((set) => ({
                ...
                updatedCvDetails: async (cvId: number, applicantId: number, data: Partial<CvResponseDto>) => {
                    set((state) => {
                        state.loading = true;
                    });
                    try {
                        const response = await privateApiClient.put<APIResponse<CvResponseDto>>(`/cv/${cvId}/update`, data,{
                            params: {
