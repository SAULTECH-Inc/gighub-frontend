...
interface InitialProfileState {
  ...
  updateBrandAndVisuals: (
    brandAndVisuals: FileUploadRequest,
...
export const useEmployerProfile = create<InitialProfileState>()(
  devtools(
    persist(
      immer<InitialProfileState>((set) => ({
        // Employer Profile
        ...
        updateCompanyInfo: async (companyInfo: CompanyInfos) => {
          try {
            const response = await privateApiClient.put<
              APIResponse<CompanyInfos>
            >(
              `${API_BASE_URL}/users/employers/update/company-info`,
              companyInfo,
            );
            set((state) => {
        ...
        updateContactInfo: async (contactInfo: ContactInfo) => {
          try {
            const response = await privateApiClient.put<
              APIResponse<ContactInfo>
            >(
              `${API_BASE_URL}/users/employers/update/contact-info`,
              contactInfo,
            );
            set((state) => {
        ...
        updateAboutCompany: async (aboutCompany: AboutCompany) => {
          try {
            const response = await privateApiClient.put<
              APIResponse<AboutCompany>
            >(
              `${API_BASE_URL}/users/employers/update/company-overview`,
              aboutCompany,
            );
            set((state) => {
        ...
        updateSocials: async (socials: Socials) => {
          try {
            const response = await privateApiClient.put<APIResponse<Socials>>(
              `${API_BASE_URL}/users/employers/update/company-socials`,
              socials,
            );
            set((state) => {
        ...
        updateComplianceAndVerification: async (
          complianceAndVerification: ComplianceAndVerifications,
        ) => {
          try {
            const response = await privateApiClient.put<
              APIResponse<ComplianceAndVerifications>
            >(
              `${API_BASE_URL}/users/employers/update/compliance-and-verification`,
              complianceAndVerification,
            );
            set((state) => {
        ...
        uploadFile: async (request: FileUploadRequest) => {
          try {
            ...
            const response = await privateApiClient.post<
              APIResponse<FileUploadResponse>
            >(`${VITE_API_FILE_SERVICE}/gighub/upload`, request, {
              headers: {
                "Content-Type": "multipart/form-data",
              },
            });
