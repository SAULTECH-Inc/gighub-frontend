...
const useSectionEditable = (sectionName: string) => {
  const isEditable = useFormStore(
    (state) => state.editableSections[sectionName] || false,
  );
  ...
  return useMemo(
    () => ({ isEditable, toggleEdit: () => toggleEdit(sectionName) }),
    [isEditable, toggleEdit, sectionName],
  );
};
