...
export const fetchTopCandidates = async (
  ...
  console.log(
    `Fetching top candidates with limit: ${limit}, status: ${status}`,
  );

  const response = await privateApiClient.get<
    APIResponse<DashboardCandidate[]>
...
export const fetchUpcomingInterviews = async (
  ...
  const response = await privateApiClient.get<
    APIResponse<DashboardInterview[]>
  >(`${API_BASE_URL}/employer/dashboard/interviews`, { params: { days } });
...
export const fetchActiveJobs = async (
  ...
  const response = await privateApiClient.get<
    APIResponse<DashboardJobPosting[]>
  >(`${API_BASE_URL}/employer/dashboard/jobs`, { params: { limit } });
...
export const getApplicationsByStatus = async (
  ...
  console.log(
    `Fetching applications count for status: ${status}, period: ${period}`,
  );
...
export const getJobViewsCount = async (
  ...
  const response =
    await privateApiClient.get<APIResponse<{ views: number }>>(endpoint);
 ...
 */
export const calculateHiringFunnel = async (
  period: string = "month",
  ...
  try {
    ...
    const allApplicationsResponse = await privateApiClient.post<
...
export const transformExistingMetrics = async (
  ...
  const [activeJobsResponse, offersSentResponse] = await Promise.all([
    privateApiClient.get<APIResponse<{ count: number }>>(
      `${API_BASE_URL}/jobs/count?status=${JobStatus.POSTED}`,
    ),
...
export const transformInterviewsToDashboard = (
  ...
  return interviews.map((interview) =>
    transformInterviewToDashboard(interview),
  );
};
...
export const transformMetricsData = (apiMetrics: DashboardMetrics) => {
  return [
    ...
    },
    {
      ...
      bgColor: "bg-teal-50",
      description:
        apiMetrics.avgTimeToHire.trend === "down"
          ? "improvement"
          : "vs last period",
    },
  ];
