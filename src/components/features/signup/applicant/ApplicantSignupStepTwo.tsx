...
const ApplicantSignupStepTwo: React.FC<StepTwoProp> = ({
    ...
    const [documentType, setDocumentType] = useState<string | undefined>(formData.documentType);
    ...
    const handleFileUpload = (files: FileList | null) => {
        if (files) {
            ...
            const newFiles = fileArray.map((file) => ({
                ...
                progress: 0,
                file,
                icon: getFileIcon(file.name), // Get the file icon based on the file name
            }));
    ...
    const getFileIcon = (fileName: string) => {
        if (fileName.toLowerCase().includes("resume")) {
            return documentAttachment; // Return the appropriate icon based on file name
        } else if (fileName.toLowerCase().includes("cover letter")) {
            return documentAttachment; // Another example
        } else if (fileName.toLowerCase().includes("video")) {
            return videoAttachment;
        }
        return documentAttachment; // Default icon for other files
    };
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <option value="employment application">Work Samples</option>
                    </select>
