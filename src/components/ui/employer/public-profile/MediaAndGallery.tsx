...
const MediaAndGallery: React.FC<MediaAndGalleryProps> = memo(({ user }) => {
  ...
  const handlePrevImage = useCallback(() => {
    if (selectedImage !== null && user?.brandAndVisuals) {
      setSelectedImage(
        selectedImage > 0 ? selectedImage - 1 : user.brandAndVisuals.length - 1,
      );
    }
  ...
  const handleNextImage = useCallback(() => {
    if (selectedImage !== null && user?.brandAndVisuals) {
      setSelectedImage(
        selectedImage < user.brandAndVisuals.length - 1 ? selectedImage + 1 : 0,
      );
    }
  }, [selectedImage, user?.brandAndVisuals]);
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {validImages.map((url, index) => (
              ...
              >
                ...
                {/* Image Number Badge */}
                <div className="bg-opacity-60 absolute right-2 bottom-2 rounded-md bg-black px-2 py-1 text-xs font-medium text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                  {index + 1}
                </div>
              </motion.div>
            ))}
        ...
        {isModalOpen &&
          ...
          validImages[selectedImage] && (
            ...
            >
              ...
              <button
                onClick={handleCloseModal}
                className="bg-opacity-50 hover:bg-opacity-70 absolute top-4 right-4 z-10 rounded-full bg-black p-2 text-white transition-colors focus:ring-2 focus:ring-white focus:outline-none"
              ...
              {validImages.length > 1 && (
                <>
                  <button
                    ...
                    className="bg-opacity-50 hover:bg-opacity-70 absolute top-1/2 left-4 z-10 -translate-y-1/2 rounded-full bg-black p-3 text-white transition-colors focus:ring-2 focus:ring-white focus:outline-none"
                  ...
                  <button
                    ...
                    className="bg-opacity-50 hover:bg-opacity-70 absolute top-1/2 right-4 z-10 -translate-y-1/2 rounded-full bg-black p-3 text-white transition-colors focus:ring-2 focus:ring-white focus:outline-none"
              ...
              >
                ...
                {/* Image Counter */}
                <div className="bg-opacity-60 absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-black px-4 py-2 text-sm font-medium text-white">
