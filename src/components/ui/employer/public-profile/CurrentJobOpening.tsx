...
const JobOpening: React.FC<JobOpeningProp> = memo(({ employerId }) => {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      const res = await fetchJobsByEmployer(employerId, pagination.page, pagination.limit);
      setJobs(res?.data || []);
    } catch (error: any) {
      const errorMessage = error?.response?.data?.message || "Failed to load job openings";
  ...
  const handleApplyClick = useCallback((job: JobPostResponse) => {
    setJobToApply(job);
  ...
  if (isLoading) {
    return (
      <div className="p-6 sm:p-8">
        <h2 className="mb-6 text-2xl font-bold text-slate-900">Current Job Openings</h2>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {[...Array(3)].map((_, index) => (
            <div key={index} className="animate-pulse rounded-xl bg-slate-100 p-6">
    ...
    return (
      <div className="p-6 sm:p-8">
        ...
        <div className="rounded-xl bg-red-50 p-8 text-center">
          ...
          <h3 className="mb-2 text-lg font-semibold text-red-800">Error Loading Jobs</h3>
      ...
      >
        ...
        {jobs.length === 0 ? (
          ...
          >
            ...
            <h3 className="mb-2 text-lg font-semibold text-slate-600">No Open Positions</h3>
            <p className="text-slate-500">This company currently has no job openings available.</p>
          </motion.div>
        ) : (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {jobs.map((job, index) => (
              ...
              >
                {/* Job Header */}
                <div className="mb-4">
                  <h3 className="mb-2 text-lg font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors line-clamp-2">
                    {job.title}
                  </h3>
                ...
                {/* Job Details */}
                <div className="mb-4 space-y-2 flex items-center flex-wrap justify-between text-sm text-slate-500 group-hover:text-indigo-600 transition-colors">
                  {job.employmentType && (
                    <div className="flex items-center space-x-2 text-sm text-slate-600">
                  ...
                  {job?.salaryRange !== null && job?.salaryRange?.maximumAmount > 0  && (
                    <div className="flex items-center space-x-2 text-sm text-slate-600">
                ...
                <div className="flex gap-3">
                  ...
                  <Link
                    ...
                    className="flex items-center justify-center space-x-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition-all hover:bg-slate-50 hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2"
                  >
